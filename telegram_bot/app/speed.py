...
class Speed(BaseApp):
    ...
    def init_regex_command(self) -> dict[re.Pattern, Callable[[Any], Coroutine[None, None, None]]]:
        ...
        return {  # type: ignore
            ...
            re.compile(f"/date_range {date_pattern} {date_pattern}"): self._get_report_date_range

        }

    @bot_d(routing_key="speed_rpc_queue")
    async def _command(
            self, command: str, **_: NewMessage.Event
    ):
        return {"report_type": command[1:]}

    @bot_d(routing_key="speed_rpc_queue")
    async def _get_report_date_range(self, command: str, start_date: str, end_date: str, **_: NewMessage.Event):
        return {"report_type": command[1:], "start_date": start_date, "end_date": end_date}
